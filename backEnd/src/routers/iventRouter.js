...
const normalizationOfWindDirection = (directionInDegrees) => {
  if (
    (directionInDegrees >= 0 && directionInDegrees < 30) ||
    (directionInDegrees > 330 && directionInDegrees <= 360)
  ) {
    return "Восток";
  }
...
router
  ...
  .get(async (req, res) => {
    try {
      const allIvents = await Ivent.find()
        .populate("passengerPending")
        .populate("passengerAccepted")
        .populate("passengerRejected")
        .populate("creator");
      res.json({ allIvents });
    } catch (error) {
  ...
  .post(async (req, res) => {
    try {
      ...
      newIvent = await Ivent.findById(newIvent._id)
        .populate("passengerPending")
        .populate("passengerAccepted")
        .populate("passengerRejected")
        .populate("creator");
  ...
  .put(async (req, res) => {
    ...
    const tomorrow = new Date(todayNotFormat.getTime() + 24 * 60 * 60 * 1000)
      .toISOString()
      .split(".")[0];
    const afterTomorrow = new Date(
      todayNotFormat.getTime() + 24 * 60 * 60 * 1000 * 2
    )
      .toISOString()
      .split(".")[0];
    const inThreeDays = new Date(
      todayNotFormat.getTime() + 24 * 60 * 60 * 1000 * 3
    )
      .toISOString()
      .split(".")[0];
  ...
  .delete(async (req, res) => {
    try {
      ...
      if (
        ...
        selectIvent.passengerPending.splice(
          selectIvent.passengerPending.indexOf(currentUserId),
          1
        );
      } else if (selectIvent.passengerAccepted.includes(currentUserId)) {
        selectIvent.passengerAccepted.splice(
          selectIvent.passengerAccepted.indexOf(currentUserId),
          1
        );
      } else if (selectIvent.passengerRejected.includes(currentUserId)) {
        selectIvent.passengerRejected.splice(
          selectIvent.passengerRejected.indexOf(currentUserId),
          1
        );
      }
...
router.route("/:lotitude/:longitude").get(async (req, res) => {
  const { lotitude, longitude } = req.params;
  try {
    const allIvents = await Ivent.find();
    const selectIventArr = allIvents.filter((elem) => {
      return (
        String(elem.coords[0]) === lotitude &&
        String(elem.coords[1]) === longitude
      );
    });
...
router
  ...
  .post(async (req, res) => {
    try {
      ...
      selectIvent.passengerPending.splice(
        selectIvent.passengerPending.indexOf(selectUserId),
        1
      );
      await selectIvent.save();
      await Ivent.updateOne(
        { _id: selectIventId },
        { $push: { passengerAccepted: selectUserId } }
      );
  ...
  .delete(async (req, res) => {
    try {
      ...
      if (selectIvent.passengerRejected.includes(selectUserId)) {
        ...
        selectIvent.passengerAccepted.splice(
          selectIvent.passengerAccepted.indexOf(selectUserId),
          1
        );
      }
      await selectIvent.save();
      await Ivent.updateOne(
        { _id: selectIventId },
        { $push: { passengerRejected: selectUserId } }
      );
  ...
  .patch(async (req, res) => {
    try {
      ...
      await Ivent.updateOne(
        { _id: selectIventId },
        { $push: { passengerPending: selectUserId } }
      );

      const selectIventWithPendingPassenger = await Ivent.findById(
        selectIventId
      )
        .populate("passengerPending")
        .populate("passengerAccepted")
